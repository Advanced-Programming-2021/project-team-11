...
public class HeraldOfCreation extends InitializableEffectMonsters {
    ...
    public boolean isConditionMade(PlayerBoard myBoard, PlayerBoard rivalBoard, PlayableCard thisCard, int activationCounter) {
        return !thisCard.hasEffectActivated() && myBoard.getGraveyard().stream().noneMatch(card -> card.getCard().getCardType() == CardType.MONSTER && ((MonsterCard) card.getCard()).getLevel() >= 7) && !myBoard.isMonsterZoneEmpty();
    }
}
